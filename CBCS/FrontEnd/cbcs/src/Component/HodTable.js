...
const HodTable = () => {
  ...
  const handleClick = async (e, user) => {
    e.preventDefault();
    const response = await fetch(`https://sathyabama-cbcs-dxbn.vercel.app/cbcs/hod/RegStudent/${user._id}`, {
  ...
  const handleDelete = async (e, user) => {
    const response = await fetch(`https://sathyabama-cbcs-dxbn.vercel.app/cbcs/hod/delete/${user._id}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${HOD.token}` },
    });
    const json = await response.json();
